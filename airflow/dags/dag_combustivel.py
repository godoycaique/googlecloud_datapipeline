...
CODE_BUCKET_NAME = 'data-pipeline-godoy-combustiveis-brasil-pyspark-code'
...
with DAG(
    ...
    get_data = SimpleHttpOperator(
        task_id='get_op',
        method='POST',
        http_conn_id='gcp-data-pipeline',
        endpoint='download_combustivel',
        data=json.dumps({
            "bucket_name": "data-pipeline-godoy-combustiveis-brasil-raw",
    ...
    PYSPARK_JOB = {
        ...
        "pyspark_job": {
            ...
            "args": [
                '--path_input',
                "gs://data-pipeline-godoy-combustiveis-brasil-raw/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/ca-{{ dag_run.logical_date.strftime('%Y') }}-{{ '01' if dag_run.logical_date.month <= 6 else '02'}}.csv",
                '--path_output',
                "gs://data-pipeline-godoy-combustiveis-brasil-curated/combustiveis-brasil/{{ dag_run.logical_date.strftime('%Y') }}/{{ '01' if dag_run.logical_date.month <= 6 else '02'}}/",
                '--file_format', 'parquet',
